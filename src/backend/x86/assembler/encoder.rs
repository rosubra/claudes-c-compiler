...
fn mnemonic_size_suffix(mnemonic: &str) -> Option<u8> {
    ...
    match mnemonic {
        ...
        | "fabs" | "fxch" | "fsubp" | "fdivp"
        | "f2xm1" | "fyl2x" | "fptan" | "fpatan" | "fprem" | "fprem1"
        | "fyl2xp1" | "fsqrt" | "fsincos" | "frndint" | "fsin" | "fcos"
        | "ftst" | "fxam" | "fld1" | "fldl2t" | "fldl2e" | "fldpi"
        | "fldlg2" | "fldln2" | "fldz" | "fscale" | "fdecstp" | "fincstp"
        | "fnop" | "fchs" => return None,
        _ => {}
    }
...
impl InstructionEncoder {
    ...
    fn encode_mnemonic(&mut self, instr: &Instruction) -> Result<(), String> {
        ...
        match mnemonic {
            ...
            // Bit test
            "btl" => self.encode_bt(ops, 4, 4),
            "btq" => self.encode_bt(ops, 4, 8),
            ...
            "bt" if infer_size_from_ops(ops).is_some() => { let s = infer_size_from_ops(ops).unwrap(); self.encode_bt(ops, 4, s) }
    ...
    fn encode_mov_rr(&mut self, src: &Register, dst: &Register, size: u8) -> Result<(), String> {
        ...
        // Handle segment register moves: MOV Sreg, r/m (8C /r) and MOV r/m, Sreg (8E /r)
        if is_segment_reg(&src.name) {
            // mov %seg, %reg -> 8C /r (seg in reg field, dst in r/m field)
            self.bytes.push(0x8C);
            self.bytes.push(self.modrm(3, src_num, dst_num));
            return Ok(());
        }
        if is_segment_reg(&dst.name) {
            // mov %reg, %seg -> 8E /r (seg in reg field, src in r/m field)
    ...
    fn encode_mov_reg_mem(&mut self, src: &Register, mem: &MemoryOperand, size: u8) -> Result<(), String> {
        ...
        // Handle mov %seg, mem -> 8C /r
        if is_segment_reg(&src.name) {
            self.bytes.push(0x8C);
            return self.encode_modrm_mem(src_num, mem);
        }
    ...
    fn encode_unary_rm(&mut self, ops: &[Operand], op_ext: u8, size: u8) -> Result<(), String> {
        ...
        // INC (/0) and DEC (/1) use opcode FE/FF, not F6/F7
        let (op8, op_wide) = if op_ext <= 1 { (0xFEu8, 0xFFu8) } else { (0xF6, 0xF7) };
        match &ops[0] {
            Operand::Register(reg) => {
                let num = reg_num(&reg.name).ok_or("bad register")?;
                if size == 2 { self.bytes.push(0x66); }
                self.emit_rex_unary(size, &reg.name);
                self.bytes.push(if size == 1 { op8 } else { op_wide });
    ...
    fn encode_bt(&mut self, ops: &[Operand], op_ext: u8, size: u8) -> Result<(), String> {
        ...
        // op_ext: 4=bt, 5=bts, 6=btr, 7=btc (matches ModRM /r field for imm form)
        // Two forms:
        //   bt $imm8, r/m  -> 0F BA /4 ib (bts=/5, btr=/6, btc=/7)
    ...
    fn emit_vex2(&mut self, r: bool, vvvv: u8, l: bool, pp: u8) {
        ...
        // r: true = not-extended (R~=1), false = extended (R~=0)
        // vvvv: raw register number (0-15), will be ones-complemented; 0 = unused (encodes as 1111b)
        let mut byte1 = 0u8;
        if r { byte1 |= 0x80; } // R~ is directly the r parameter
        byte1 |= ((!vvvv) & 0xF) << 3;
        if l { byte1 |= 0x04; }
        byte1 |= pp & 3;
    ...
    fn emit_vex3(&mut self, r: bool, x: bool, b: bool, mmmmm: u8, w: bool, vvvv: u8, l: bool, pp: u8) {
        ...
        // r/x/b: true = not-extended (bit=1), false = extended (bit=0)
        // vvvv: raw register number (0-15), will be ones-complemented; 0 = unused (encodes as 1111b)
        let mut byte1 = 0u8;
        if r { byte1 |= 0x80; }
        if x { byte1 |= 0x40; }
        ...
        match (&ops[0], &ops[1]) {
                ...
                if (is_xmm(&src.name) || is_ymm(&src.name)) && (is_xmm(&dst.name) || is_ymm(&dst.name)) =>
            {
                ...
                if b {
                    ...
                    self.emit_vex3(r, true, b, 1, false, 0, l, pp);
                }
                ...
                if is_xmm(&dst.name) || is_ymm(&dst.name) =>
            {
                ...
                if b_ext && x {
                    ...
                    self.emit_vex3(r, x, b_ext, 1, false, 0, l, pp);
                }
                self.bytes.push(load_op);
                self.encode_modrm_mem(dst_num, mem)
            }
    ...
    fn encode_vex_unary_imm8_w1(&mut self, ops: &[Operand], prefix: u8, opcode: u8) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1], &ops[2]) {
            (Operand::Immediate(ImmediateValue::Integer(imm)), Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&dst.name);
                let b = !needs_rex_ext(&src.name);
                self.emit_vex3(r, true, b, 3, true, 0, l, pp); // mmmmm=3 for 0F 3A, W=1
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                self.bytes.push(*imm as u8);
                Ok(())
    ...
    fn encode_vex_shift(&mut self, ops: &[Operand], prefix: u8, reg_op: u8, imm_ext: u8, imm_op: u8) -> Result<(), String> {
        if ops.len() != 3 {
    ...
    fn encode_vex_unary_38(&mut self, ops: &[Operand], prefix: u8, opcode: u8) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1]) {
            ...
            (Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&dst.name);
                let b = !needs_rex_ext(&src.name);
                self.emit_vex3(r, true, b, 2, false, 0, l, pp);
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                Ok(())
            }
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src), Operand::Register(dst)) => {
                ...
                if b {
                    ...
                    self.emit_vex3(r, true, b, 1, false, 0, false, pp);
                }
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                Ok(())
            ...
            (Operand::Memory(mem), Operand::Register(dst)) => {
                ...
                if b_ext && x {
                    ...
                    self.emit_vex3(r, x, b_ext, 1, false, 0, false, pp);
                }
                self.bytes.push(opcode);
                self.encode_modrm_mem(dst_num, mem)
            }
    ...
    fn encode_vex_extract128(&mut self, ops: &[Operand], prefix: u8, opcode: u8) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1], &ops[2]) {
            (Operand::Immediate(ImmediateValue::Integer(imm)), Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&src.name);
                let b = !needs_rex_ext(&dst.name);
                self.emit_vex3(r, true, b, 3, false, 0, true, pp); // L=1 for 256-bit
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, src_num, dst_num));
                self.bytes.push(*imm as u8);
                Ok(())
    ...
    fn encode_vex_movd(&mut self, ops: &[Operand]) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src), Operand::Register(dst)) if !is_xmm(&src.name) && is_xmm(&dst.name) => {
                ...
                let b = !needs_rex_ext(&src.name);
                if b {
                    self.emit_vex2(r, 0, false, 1); // pp=1 for 66
                } else {
                    self.emit_vex3(r, true, b, 1, false, 0, false, 1);
                }
            ...
            (Operand::Memory(mem), Operand::Register(dst)) if is_xmm(&dst.name) => {
                ...
                if b_ext && x {
                    ...
                    self.emit_vex3(r, x, b_ext, 1, false, 0, false, 1);
                }
                self.bytes.push(0x6E);
                self.encode_modrm_mem(dst_num, mem)
            }
    ...
    fn encode_vex_movq(&mut self, ops: &[Operand]) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src), Operand::Register(dst)) if !is_xmm(&src.name) && is_xmm(&dst.name) => {
                ...
                let r = !needs_rex_ext(&dst.name);
                let b = !needs_rex_ext(&src.name);
                self.emit_vex3(r, true, b, 1, true, 0, false, 1); // W=1
                self.bytes.push(0x6E);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                Ok(())
            }
            (Operand::Register(src), Operand::Register(dst)) if is_xmm(&src.name) && !is_xmm(&dst.name) => {
                ...
                let r = !needs_rex_ext(&src.name);
                let b = !needs_rex_ext(&dst.name);
                self.emit_vex3(r, true, b, 1, true, 0, false, 1);
                self.bytes.push(0x7E);
                self.bytes.push(self.modrm(3, src_num, dst_num));
                Ok(())
            }
            (Operand::Register(src), Operand::Register(dst)) if is_xmm(&src.name) && is_xmm(&dst.name) => {
                ...
                let b = !needs_rex_ext(&src.name);
                if b {
                    self.emit_vex2(r, 0, false, 2); // pp=2 for F3
                } else {
                    self.emit_vex3(r, true, b, 1, false, 0, false, 2);
                }
                self.bytes.push(0x7E);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                Ok(())
            ...
            (Operand::Memory(mem), Operand::Register(dst)) if is_xmm(&dst.name) => {
                ...
                if b_ext && x {
                    ...
                    self.emit_vex3(r, x, b_ext, 1, false, 0, false, 2);
                }
                self.bytes.push(0x7E);
                self.encode_modrm_mem(dst_num, mem)
            }
    ...
    fn encode_vex_cvt_to_gp(&mut self, ops: &[Operand], prefix: u8, opcode: u8, is_64: bool) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1]) {
            (Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&dst.name);
                let b = !needs_rex_ext(&src.name);
                self.emit_vex3(r, true, b, 1, is_64, 0, false, pp);
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                Ok(())
            }
    ...
    fn encode_vex_extract_gp_3a(&mut self, ops: &[Operand], prefix: u8, opcode: u8) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1], &ops[2]) {
            (Operand::Immediate(ImmediateValue::Integer(imm)), Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&src.name);
                let b = !needs_rex_ext(&dst.name);
                self.emit_vex3(r, true, b, 3, false, 0, false, pp);
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, src_num, dst_num));
                self.bytes.push(*imm as u8);
                Ok(())
    ...
    fn encode_vex_extract_gp_3a_w1(&mut self, ops: &[Operand], prefix: u8, opcode: u8) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1], &ops[2]) {
            (Operand::Immediate(ImmediateValue::Integer(imm)), Operand::Register(src), Operand::Register(dst)) => {
                ...
                let r = !needs_rex_ext(&src.name);
                let b = !needs_rex_ext(&dst.name);
                self.emit_vex3(r, true, b, 3, true, 0, false, pp);
                self.bytes.push(opcode);
                self.bytes.push(self.modrm(3, src_num, dst_num));
                self.bytes.push(*imm as u8);
                Ok(())
    ...
    /// FSUBP - AT&T fsubp = DE E0+i (no operands = fsubp %st, %st(1) = DE E1)
    /// With operands: fsubp %st, %st(i) = DE E0+i
    fn encode_fsubp(&mut self, ops: &[Operand]) -> Result<(), String> {
        // AT&T fsubp = DE E0+i (GAS/AT&T semantics: st(i) = st(i) - st(0), pop)
        // Default: fsubp %st, %st(1) = DE E1
        if ops.is_empty() {
        ...
        if ops.len() == 2 {
            ...
            match &ops[1] {
                Operand::Register(reg) => {
                    let n = parse_st_num(&reg.name)?;
                    self.bytes.extend_from_slice(&[0xDE, 0xE0 + n]);
                    Ok(())
                }
    ...
    /// FDIVP - AT&T fdivp = DE F0+i (GAS/AT&T semantics: st(i) = st(i) / st(0), pop)
    /// Default: fdivp %st, %st(1) = DE F1
    fn encode_fdivp(&mut self, ops: &[Operand]) -> Result<(), String> {
        ...
        if ops.len() == 2 {
            match &ops[1] {
                Operand::Register(reg) => {
                    let n = parse_st_num(&reg.name)?;
                    self.bytes.extend_from_slice(&[0xDE, 0xF0 + n]);
                    Ok(())
                }
    ...
    fn encode_rorx(&mut self, ops: &[Operand], mnemonic: &str) -> Result<(), String> {
        ...
        match (&ops[0], &ops[1], &ops[2]) {
            (Operand::Immediate(ImmediateValue::Integer(imm)), Operand::Register(src), Operand::Register(dst)) => {
                ...
                self.emit_vex3(r, true, b, 3, is_64, 0, false, 3);
                self.bytes.push(0xF0);
                self.bytes.push(self.modrm(3, dst_num, src_num));
                self.bytes.push(*imm as u8);
                Ok(())
